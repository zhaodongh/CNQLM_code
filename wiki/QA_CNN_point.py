...
class QA_quantum(object):
    ...
    def add_embeddings(self):
        ...
        with tf.name_scope("embedding"):
            if self.is_Embedding_Needed:
                ...
                # W_complex=tf.Variable(np.array(self.embeddings_complex),name="W" ,dtype="float32",trainable = True)
                W_complex=tf.Variable(self.Position_Embedding(self.embedding_size),name = 'W',trainable = True)
            else:
                W = tf.Variable(tf.random_uniform([self.vocab_size, self.embedding_size], -1.0, 1.0),name="W",trainable = self.trainable)
                W_complex = tf.Variable(tf.random_uniform([self.vocab_size, self.embedding_size], 0, 2*math.pi),name="W",trainable = self.trainable)
            self.embedding_W = W
            ...
            tf.summary.histogram('weight_embedding',self.embedding_W_complex)

        self.embedded_chars_q,self.embedding_chars_q_complex = self.concat_embedding(self.question,self.q_position,self.q_overlap)
        print(self.embedded_chars_q)
        self.embedded_chars_a,self.embedding_chars_a_complex= self.concat_embedding(self.answer,self.a_position,self.a_overlap)
    ...
    def trace_represent(self):
        self.density_diag_r = tf.matrix_diag_part(self.M_qa_real)
        self.density_trace_r = tf.expand_dims(tf.trace(self.M_qa_real),-1)        
        self.density_diag_i = tf.matrix_diag_part(self.M_qa_imag)
        self.density_trace_i = tf.expand_dims(tf.trace(self.M_qa_imag),-1)
        self.match_represent = tf.concat([self.density_diag_r,self.density_trace_r,self.density_diag_i,self.density_trace_i],1)
    def density_matrix(self,sentence_matrix,sentence_matrix_complex,sentence_weighted):

        sentence_matrix_norm=tf.norm(sentence_matrix,ord="euclidean",axis=2)
        sentence_weighted_compute=tf.nn.softmax(sentence_matrix_norm,1)
        sentence_weighted_compute=tf.expand_dims(sentence_weighted_compute,-1)
        sentence_weighted_compute=tf.expand_dims(sentence_weighted_compute,-1)
        # sentence_matrix_norm_complex=tf.norm(sentence_matrix_complex,ord="euclidean",axis=2)
        # sentence_weighted_compute_complex=tf.nn.softmax(sentence_matrix_norm_complex,1)
        # sentence_weighted_compute_complex=tf.expand_dims(sentence_weighted_compute_complex,-1)
        # sentence_weighted_compute_complex=tf.expand_dims(sentence_weighted_compute_complex,-1)
        ...
        return tf.reduce_sum(tf.multiply(q_a_real,sentence_weighted_compute),1),tf.reduce_sum(tf.multiply(q_a_imag,sentence_weighted_compute),1)
    ...
    def concat_embedding(self,words_indice,position_indice,overlap_indice):
        ...
        self.embedding_chars_q_phase=tf.nn.embedding_lookup(self.embedding_W_complex,position_indice)
        # self.embedding_chars_q_phase=tf.nn.embedding_lookup(self.position_W,position_indice)
        embedding_chars_q_phase=tf.nn.dropout(self.embedding_chars_q_phase, self.dropout_keep_prob, name="hidden_output_drop")
        [embedded_chars_q, embedding_chars_q_phase] = ComplexMultiply()([self.embedding_chars_q_phase,embedded_chars_q])
